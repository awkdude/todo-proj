...
export function createTaskElement(task) {
    ...
    if (!isTaskLate(task)) {
        ...
        new_task_item.append(input_element);
        // }}}
    } else {
        new_task_item.classList.add('late');
        const task_status = document.createElement('label');
        task_status.classList.add('task-status');
        if (task.is_range) {
            new_task_item.textContent = `${task.completion_value} / ${task.range_max}`;
        } else {
            new_task_item.textContent =
                task.completion_value > 0 ? 'COMPLETED' : 'Incomplete';
        }
        new_task_item.append(task_status);
    }
    let label_element = document.createElement('label');
    label_element.classList.add('tasklabel');
